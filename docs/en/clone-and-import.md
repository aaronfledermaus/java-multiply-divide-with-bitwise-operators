...
`git clone https://github.com/ByteLegendQuest/java-multiply-divide-with-bitwise-operators`

A project directory named `java-multiply-divide-with-bitwise-operators` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-multiply-divide-with-bitwise-operators` directory which is created just now.
